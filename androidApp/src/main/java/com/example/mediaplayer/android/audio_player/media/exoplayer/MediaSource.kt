package com.example.mediaplayer.android.audio_player.media.exoplayer

import android.support.v4.media.MediaBrowserCompat
import android.support.v4.media.MediaBrowserCompat.MediaItem.FLAG_PLAYABLE
import android.support.v4.media.MediaDescriptionCompat
import android.support.v4.media.MediaMetadataCompat
import com.example.mediaplayer.android.audio_player.data.repository.AudioRepository
...
class MediaSource @Inject constructor(private val repository: AudioRepository) {
    private val onReadyListerners: MutableList<OnReadyListerner> = mutableListOf()
    ...
    private var state: AudioSourceState = AudioSourceState.STATE_CREATED
        set(value) {
            if (value == AudioSourceState.STATE_CREATED || value == AudioSourceState.STATE_ERROR) {
                synchronized(onReadyListerners) {
                    field = value
                    onReadyListerners.forEach { listener: OnReadyListerner ->
    ...
    fun refresh(){
        onReadyListerners.clear()
        state = AudioSourceState.STATE_CREATED
    }


    fun whenReady(listener: OnReadyListerner): Boolean {
        //STATE_INITIALIZED ==> STATE_INITIALIZING
        return if (state == AudioSourceState.STATE_CREATED || state == AudioSourceState.STATE_INITIALIZED) {
            onReadyListerners += listener
            false
        } else {
            listener.invoke(isReady)
            true
    ...
    private val isReady: Boolean
        get() = state == AudioSourceState.STATE_INITIALIZE
...
typealias OnReadyListerner = (Boolean) -> Unit